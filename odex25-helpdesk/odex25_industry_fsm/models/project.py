...
class Task(models.Model):
    ...
    def _read_group_user_ids(self, users, domain, order):
        if self.env.context.get('fsm_mode'):
            ...
            search_domain = ['|', '|', ('id', 'in', users.ids), ('groups_id', 'in', self.env.ref('odex25_industry_fsm.group_fsm_user').id), ('id', 'in', recently_created_tasks.mapped('user_id.id'))]
    ...
    def action_view_timesheets(self):
        ...
        form_view = self.env.ref('odex25_industry_fsm.timesheet_view_form')
        tree_view = self.env.ref('odex25_industry_fsm.timesheet_view_tree_user_inherit')
        return {
    ...
    def action_fsm_view_overlapping_tasks(self):
        ...
        fsm_task_list_view = self.env.ref('odex25_industry_fsm.project_task_view_list_fsm')
        fsm_task_kanban_view = self.env.ref('odex25_industry_fsm.project_task_view_kanban_fsm')
