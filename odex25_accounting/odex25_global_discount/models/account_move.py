...
class AccountMove(models.Model):
    ...
    def _compute_amount(self):
        ...
        sale_account_id = self.env['ir.config_parameter'].sudo().get_param(
            'odex25_global_discount.sale_account_id')
        purchase_account_id = self.env['ir.config_parameter'].sudo().get_param(
            'odex25_global_discount.purchase_account_id')
                ...
                if tax_discount_policy == 'tax':
                    if rec.discount_type == 'line':
                        ...
                        if self._context.get('default_move_type') == 'in_invoice':
                            for line in self.invoice_line_ids:
                                if line.discount_method == 'per':
                                    ...
                                    total += (line.price_subtotal + price_tax) * (line.discount_amount / 100)
                        ...
                        if rec.discount_method == 'fix':
                            ...
                            rec.amount_total = rec.amount_untaxed - rec.discount_amt
                    ...
                    if tax_discount_policy == 'untax':
                        if rec.discount_type == 'line':
                            if base_line.discount_method == 'fix':
                                if base_line.discount_amount > 0:
                                    price_unit_wo_discount = base_line.price_unit - rec.currency_id.round(
                                        base_line.discount_amount / base_line.quantity)
                                    if move.is_invoice(include_receipts=True):
                                        sign = -1 if move.is_inbound() else 1
                                ...
                                if base_line.discount_amount > 0:
                                    price_unit_wo_discount = base_line.price_unit * (
                            ...
                            if rec.discount_amount > 0:
                                if rec.amount_untaxed != 0.0:
                                    final_discount = ((rec.discount_amt * base_line.price_unit) / rec.amount_untaxed)
                                    price_unit_wo_discount = base_line.price_unit - rec.currency_id.round(
                                        final_discount)
                                    if move.is_invoice(include_receipts=True):
                                        quantity = base_line.quantity
    ...
    def create(self, vals_list):
        ...
        for val in vals_list:
            if val.get('flag') and not self._context.get('default_move_type') == 'in_invoice':
                val.pop('flag')
            else:
                for rec in res:
                    name = False
                    ...
                    if name != 'Discount':
                        if rec.discount_account_id:
                            self.env['account.move.line'].with_context(check_move_validity=False).create({
                                'account_id': rec.discount_account_id.id,
                                'quantity': 1,
                                'price_unit': -price,
                                'move_id': rec.id,
                                'name': "Discount",
                            ...
                            pass
        res.with_context(check_move_validity=False)._onchange_recompute_dynamic_lines()
        return res
...
class AccountMoveLine(models.Model):
    ...
    @api.onchange('discount_method', 'discount_amount', 'amount_currency', 'currency_id', 'debit', 'credit', 'tax_ids',
                  'account_id', )
