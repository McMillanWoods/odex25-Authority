...
class PettyCash(models.Model):
    ...
    account_id = fields.Many2one(comodel_name="account.account", string="Petty Cash Account", )
    ...
    total_replace_amount = fields.Float(string="Total Petty Amount", compute="_compute_total_replace_amount",
                                        readonly=True, )
    petty_cash_remaining = fields.Float(string="Remaining", compute="_compute_petty_cash_remaining", readonly=True, )
    ...
    type = fields.Selection(selection=[('continuous', 'Continuous'), ('temporary', 'Temporary')], )
    ...
    def _compute_petty_cash_expenses(self):
        for rec in self:
            rec.petty_cash_expenses = sum(self.env['hr.expense.sheet'].search(
                [('petty_cash_id', '=', self.id)]).filtered(lambda line: line.petty_cash_type != 'renew').mapped(
                'total_amount'))
