...
class PettyCash(models.Model):
    _name = 'petty.cash'
    _inherit = ['petty.cash', 'mail.thread', 'mail.activity.mixin']
...
class CrossoveredBudgetLine(models.Model):
    ...
    @api.model
    def create(self, values):
        body = _("<p>Budget Lines Created</p><ul>")
        fields = self.fields_get()
        for k, v in values.items():
            field = fields.get(k)
            if field['type'] == 'selection':
                ...
                val = self.env[field['relation']].sudo().browse(v).name_get()
            else:
                val = v
            body += "<li>%s: %s</li>" % (field['string'], val)
        body += "</ul>"
        self.env['mail.message'].create({
            ...
            'res_id': values.get('crossovered_budget_id'),
            'subtype_id': '2',
        })
        return super(CrossoveredBudgetLine, self).create(values)

    def write(self, values):
        body = _("<p>Budget Line updated</p><ul>")
        fields = self.fields_get()
        old_values = self.read(values.keys())[0]
        for k, v in values.items():
            field = fields.get(k)
            # print(field['type'], "field['type']")
            # print(dict(field['selection'])[v], 'lkll')
            if field['type'] == 'selection':
                val = dict(field['selection'])[v]
                old_val = dict(field['selection'])[old_values[k]]
            elif field['type'] == 'many2one':
        ...
        return super(CrossoveredBudgetLine, self).write(values)

    def unlink(self):
        self.env['mail.message'].create({
            'body': _("<p>Budget Line deleted</p>"),
            'model': 'crossovered.budget',
            'res_id': self.crossovered_budget_id.id,
            'subtype_id': '2',
        })
        return super(CrossoveredBudgetLine, self).unlink()
...
class BudgetConfirmationLine(models.Model):
    ...
    def create(self, values):
        ...
        self.env['mail.message'].create({
            'body': body,
            'model': 'budget.confirmation',
            'res_id': values.get('confirmation_id'),
            'subtype_id': '2',
        })
        return super(BudgetConfirmationLine, self).create(values)
    ...
    def write(self, values):
        ...
        return super(BudgetConfirmationLine, self).write(values)
    ...
    def unlink(self):
        ...
        return super(BudgetConfirmationLine, self).unlink()
