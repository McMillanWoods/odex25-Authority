...
class TaxDetailsXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, taxes):
        ...
        tax_lines = self.invoice_tax(date_from, date_to, government_type, branch_ids)

        modules = self.env['ir.module.module'].sudo().search([('state', '=', 'installed'),
                                                              ('name', '=', 'hr_expense_petty_cash')])
        if modules:
            tax_lines += self.petty_tax(date_from, date_to, government_type, branch_ids)

        modules = self.env['ir.module.module'].sudo().search([('state', '=', 'installed'),
                                                              ('name', '=', 'point_of_sale')])
        if modules:
            tax_lines += self.pos_order_tax(date_from, date_to, government_type, branch_ids)

        self.report_generation(workbook, date_from, date_to, tax_lines)

    def invoice_tax(self, date_from, date_to, government_type=False, branch_ids=False):
        domain = [
        ...
        for line in taxed_lines:
            for tax in line.tax_ids:
                ...
                if tax_discount_policy and tax_discount_policy == 'untax':
                    # if line.move_id.discount_method == 'line':
                    #     if line.discount_method == 'per':
                    #         discount = line.price_subtotal * (line.discount_amount / 100)
                    #     else:
                    #         discount = (line.discount_amount / line.price_subtotal) * 100
                    #     taxes = \
                    #         tax.compute_all(discount, line.currency_id,
                    #                         line.quantity,
                    #                         line.product_id, line.partner_id)['taxes']
                    # else:
                    #     if line.move_id.discount_method == 'fix':
                    if line.move_id.discount_amt and line.move_id.amount_untaxed:
                        discount = line.move_id.discount_amt / line.move_id.amount_untaxed
                        price = ((1 - discount) * line.price_unit)
                    else:
                        price = line.price_unit

                    taxes = tax.compute_all(price, line.currency_id,
                                            line.quantity,
                                            line.product_id, line.partner_id)['taxes']

                    # in case of create tax from bank statement
                    if line.move_id.move_type == 'entry' and line.statement_id:
                        for tt in taxes:
                            print(line.before_tax_amount, 'line.before_tax_amount')
                            tt['amount'] = abs(line.tax_ids.amount / 100) * line.before_tax_amount
                            tt['base'] = line.before_tax_amount
                    print(taxes, 'taxes')

                    if line.move_id.discount_method == 'per':
                        for t in taxes:
                            t['base'] = line.price_subtotal
                            t['discount'] = line.move_id.discount_amt
                        # else:
                        #     discount = line.move_id.discount_amount
                        #     taxes = tax.compute_all(((1 - discount) * line.price_unit), line.currency_id,
                        #                             line.quantity,
                        #                             line.product_id, line.partner_id)['taxes']
                else:
                    taxes = tax.compute_all(line.price_unit, line.currency_id, line.quantity,
                                            line.product_id, line.partner_id)['taxes']
                ...
                for t in taxes:
                    if key not in tax_grouped:
                        tax_grouped[key] = {
                            ...
                            'desc': line.product_id.name or line.name,
                            ...
                            'amount_exclude': abs(t.get('base', 0)),
                            'amount_tax': abs(t.get('amount', 0)),
                            'total_amount': abs(t.get('base', 0) + t.get('amount', 0))
                        }
                    else:
                        tax_grouped[key]['amount_exclude'] += abs(t.get('base', 0))
                        tax_grouped[key]['amount_tax'] += abs(t.get('amount', 0))
                        tax_grouped[key]['total_amount'] += abs(
                            t.get('base', 0) + t.get('amount', 0) - abs(t.get('discount', 0)))

        return list(tax_grouped.values())

    def petty_tax(self, date_from, date_to, government_type=False, branch_ids=False):
            ...
            for tax in line.tax_ids:
                ...
                for t in taxes:
                    if key not in tax_grouped:
                        ...
                        tax_grouped[key]['amount_exclude'] += abs(t.get('base', 0))
                        tax_grouped[key]['amount_tax'] += abs(t.get('amount', 0))
                        tax_grouped[key]['total_amount'] += abs(t.get('base', 0) + t.get('amount', 0))

        return list(tax_grouped.values())

    def pos_order_tax(self, date_from, date_to, government_type, branch_ids=False):
        ...
        if government_type:
            domain.append(('tax_ids.government_type', '=', government_type))
        if branch_ids:
            domain.append(('order_id.branch_id', 'in', branch_ids))
    ...
    def report_generation(self, workbook, date_from, date_to, tax_lines):
        ...
        sale_purchase_list = {'sale': [], 'purchase': [], 'entry': []}
        ...
        for item in grouped_list:
            ...
            item_sheet.write(5, 5, 'Amount exclusive of VAT المبالغ الخاضعة للضريبة الأساسية ',
                             item_format1)
            item_sheet.write(5, 6, 'VAT Amount قيمة الضريبة ', item_format1)
            item_sheet.write(5, 7, 'Total إجمالي ', item_format1)
            item_sheet.write(5, 8, 'ملاحظات Remarks', item_format1)
            row = 6
            sale_amount_exclude = 0
            sale_amount_tax = 0
            sale_adjustment = 0
            entry_amount_exclude = 0
            entry_amount_tax = 0
            entry_adjustment = 0
            ...
            for record in item['values']:
                ...
                item_sheet.write(row, 5, record.get('amount_exclude'), lines_format4)
                item_sheet.write(row, 6, record.get('amount_tax'), lines_format4)
                item_sheet.write(row, 7, record.get('total_amount'), lines_format4)
                item_sheet.write(row, 8, '', lines_format4)
                row += 1
                ...
                elif record.get('record_type') == 'entry':
                    entry_amount_exclude += record.get('amount_exclude')
                    entry_amount_tax += record.get('amount_tax')
            item_sheet.merge_range(row, 0, row, 4, 'Total إجمالي ', item_format1)
            item_sheet.write(row, 5, sale_amount_exclude + purchase_amount_exclude + entry_amount_exclude, item_format1)
            item_sheet.write(row, 6, sale_amount_tax + purchase_amount_tax + entry_amount_tax, item_format1)
            item_sheet.write(row, 7,
                             sale_amount_exclude + purchase_amount_exclude + sale_amount_tax + purchase_amount_tax + entry_amount_tax + entry_amount_exclude,
                             item_format1)
            item_sheet.write(row, 8, '', item_format1)
            row += 2
            ...
            item_sheet.write(row, 5, 'Amount exclusive of VAT المبالغ الخاضعة للضريبة الأساسية ',
                             item_format1)
            item_sheet.write(row, 6, 'VAT Amount قيمة الضريبة ', item_format1)
            item_sheet.write(row, 7, 'Total إجمالي ', item_format1)
            item_sheet.write(row, 8, 'ملاحظات Remarks', item_format1)
            row += 1
            total_exclude = 0
            for record in filter(lambda x: x.get('record_type') in ['out_refund', 'in_refund'], item['values']):
                ...
                if record.get('record_type') == 'out_refund':
                    if item_name == 'ضرائب العمولات البنكية':
                        entry_adjustment += record.get('amount_tax')
            ...
            item_sheet.merge_range(row, 0, row, 4, 'Total إجمالي ', item_format1)
            item_sheet.write(row, 5, total_exclude, item_format1)
            item_sheet.write(row, 6, sale_adjustment + purchase_adjustment + entry_adjustment, item_format1)
            item_sheet.write(row, 7, total_exclude + sale_adjustment + purchase_adjustment + entry_adjustment,
                             item_format1)
            item_sheet.write(row, 8, '', item_format1)
            ...
            if entry_amount_exclude > 0 or entry_amount_tax > 0 or entry_adjustment > 0:
                # print(entry_amount_exclude, 'amount_exclude')
                sale_purchase_list['entry'] += [{'tax_name': item_name,
                                                 'amount_exclude': entry_amount_exclude,
                                                 'amount_adjustment': entry_adjustment,
                                                 'amount_tax': entry_amount_tax}]
        # Total Sheet
        column = 6
        start_value = 6
        ...
        total_sheet.write('G%s' % (column), sale_total_tax - abs(sale_total_adjustment), total_format1)
        column += 1
        ...
        total_sheet.write('G%s' % (column), purchase_total_tax - abs(purchase_total_adjustment), total_format2)

        column += 1
        starte_value = column
        entry_total_exclude = 0
        entry_total_adjustment = 0
        entry_total_tax = 0
        for entry_item in sale_purchase_list['entry']:
            print(entry_item.get('amount_exclude'), 'vvvvvvvvvvvvv')
            total_sheet.merge_range('B%s:D%s' % (column, column), '', lines_format2)
            total_sheet.write('B%s' % (column), entry_item.get('tax_name') or 'Undefiend',
                              lines_format2)
            total_sheet.write('E%s' % (column), entry_item.get('amount_exclude'), lines_format2)
            total_sheet.write('F%s' % (column), entry_item.get('amount_adjustment'), lines_format2)
            total_sheet.write('G%s' % (column), entry_item.get('amount_tax'), lines_format2)
            entry_total_exclude += entry_item.get('amount_exclude')
            entry_total_adjustment += entry_item.get('amount_adjustment')
            entry_total_tax += entry_item.get('amount_tax')
            column += 1
        total_sheet.set_row(column, 30)
        total_sheet.merge_range('A%s:A%s' % (starte_value, column + 1), 'Vat On Entry', vat_format2)
        total_sheet.merge_range('B%s:D%s' % (column, column), 'Total Entry إجمالي القيود', total_format2)
        total_sheet.write('E%s' % (column), entry_total_exclude, total_format2)
        total_sheet.write('F%s' % (column), entry_total_adjustment, total_format2)
        total_sheet.write('G%s' % (column), entry_total_tax, total_format2)
        column += 1
        total_sheet.merge_range('B%s:F%s' % (column, column), 'Net Entry Tax صافي ضريبة القيود', total_format2)
        total_sheet.write('G%s' % (column), entry_total_tax - abs(entry_total_adjustment), total_format2)
        column += 1
        total_sheet.merge_range('A%s:F%s' % (column, column), "", total_format2)
        total_sheet.write('G%s' % (column), (entry_total_tax - abs(entry_total_adjustment)) + (
                purchase_total_tax - abs(purchase_total_adjustment)), total_format2)
        column += 1
        startf_value = column
        total_sheet.merge_range('B%s:F%s' % (column, column),
                                'Total Vat due to current period اجمالي ضريبة القيمة المضافة المستحقة للفترة',
                                lines_format3)
        total_sheet.write('G%s' % (column),abs((entry_total_tax - abs(entry_total_adjustment)) + (
                purchase_total_tax - abs(purchase_total_adjustment))
                          - (sale_total_tax - abs(sale_total_adjustment))),
                          lines_format3)
        column += 1
        total_sheet.merge_range('B%s:F%s' % (column, column),
        ...
        total_sheet.write('G%s' % (column),
                          abs((entry_total_tax - abs(entry_total_adjustment)) + (
                                  purchase_total_tax - abs(purchase_total_adjustment))
                          - (sale_total_tax - abs(sale_total_adjustment))),
                          total_format3)
        total_sheet.set_row(column, 30)
