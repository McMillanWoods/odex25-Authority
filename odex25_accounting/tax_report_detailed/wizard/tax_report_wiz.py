...
class TaxReportWiz(models.TransientModel):
    ...
    def compute_table_data(self):
        ...
        for invoice in tax_details.invoice_tax(date_from, date_to, self.government_type, self.branch_ids.ids):
            report.create(invoice)

        modules = self.env['ir.module.module'].sudo().search([('state', '=', 'installed'),
                                                              ('name', '=', 'hr_expense_petty_cash')])
        if modules:
            for petty in tax_details.petty_tax(date_from, date_to, self.government_type, self.branch_ids.ids):
                report.create(petty)

        modules = self.env['ir.module.module'].sudo().search([('state', '=', 'installed'),
                                                              ('name', '=', 'point_of_sale')])
        if modules:
            for pos in tax_details.pos_order_tax(date_from, date_to, self.government_type, self.branch_ids.ids):
                report.create(pos)
