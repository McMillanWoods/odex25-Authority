...
class Employee(models.Model):
    ...
    def _planning_get_url(self, planning):
        ...
        for employee in self:
            if employee.user_id and employee.user_id.has_group('odex25_planning.group_planning_user'):
                result[employee.id] = '/web?#action=planning.planning_action_open_shift'
            else:
                result[employee.id] = '/odex25_planning/%s/%s' % (planning.access_token, employee.employee_token)
        return result

    def _slot_get_url(self):
        action_id = self.env.ref('odex25_planning.planning_action_open_shift').id
        menu_id = self.env.ref('odex25_planning.planning_menu_root').id
    ...
    def action_view_planning(self):
        action = self.env["ir.actions.actions"]._for_xml_id("odex25_planning.planning_action_schedule_by_employee")
        action.update({
