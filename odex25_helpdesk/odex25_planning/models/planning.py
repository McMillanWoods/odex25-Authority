...
class Planning(models.Model):
    ...
    def _send_slot(self, employee_ids, start_datetime, end_datetime, include_unassigned=True, message=None):
        ...
        employee_with_backend = employee_ids.filtered(lambda e: e.user_id and e.user_id.has_group('odex25_planning.group_planning_user'))
        ...
        template = self.env.ref('odex25_planning.email_template_slot_single')
        ...
        if self.employee_id:
            ...
            if self.allow_self_unassign:
                if employee_ids.filtered(lambda e: e.user_id and e.user_id.has_group('odex25_planning.group_planning_user')):
                    unavailable_link = '/odex25_planning/unassign/%s/%s' % (self.employee_id.sudo().employee_token, self.id)
                else:
                    unavailable_link = '/odex25_planning/%s/%s/unassign/%s?message=1' % (planning.access_token, self.employee_id.sudo().employee_token, self.id)
        ...
        for employee in employee_ids.filtered(lambda e: e.work_email):
            if not self.employee_id and employee in employee_with_backend:
                view_context.update({'available_link': '/odex25_planning/assign/%s/%s' % (employee.sudo().employee_token, self.id)})
            elif not self.employee_id:
                view_context.update({'available_link': '/odex25_planning/%s/%s/assign/%s?message=1' % (planning.access_token, employee.sudo().employee_token, self.id)})
...
class PlanningPlanning(models.Model):
    ...
    def _send_planning(self, message=None, employees=False):
        ...
        for planning in self:
            ...
            template = self.env.ref('odex25_planning.email_template_planning_planning', raise_if_not_found=False)
            template_context = {
