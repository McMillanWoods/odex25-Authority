...
class ShiftController(http.Controller):
    ...
    @http.route(['/odex25_planning/<string:planning_token>/<string:employee_token>'], type='http', auth="public", website=True)
    def planning(self, planning_token, employee_token, message=False, **kwargs):
        ...
        return request.render('odex25_planning.period_report_template', planning_data)
    ...
    @http.route('/odex25_planning/<string:token_planning>/<string:token_employee>/assign/<int:slot_id>', type="http", auth="public", website=True)
    def planning_self_assign(self, token_planning, token_employee, slot_id, message=False, **kwargs):
        ...
        if slot_sudo.employee_id:
            return redirect('/odex25_planning/%s/%s?message=%s' % (token_planning, token_employee, 'already_assign'))

        slot_sudo.write({'employee_id': employee_sudo.id})
        if message:
            return redirect('/odex25_planning/%s/%s?message=%s' % (token_planning, token_employee, 'assign'))
        else:
            return redirect('/odex25_planning/%s/%s' % (token_planning, token_employee))

    @http.route('/odex25_planning/<string:token_planning>/<string:token_employee>/unassign/<int:shift_id>', type="http", auth="public", website=True)
    def planning_self_unassign(self, token_planning, token_employee, shift_id, message=False, **kwargs):
        ...
        if message:
            return redirect('/odex25_planning/%s/%s?message=%s' % (token_planning, token_employee, 'unassign'))
        else:
            return redirect('/odex25_planning/%s/%s' % (token_planning, token_employee))

    @http.route('/odex25_planning/assign/<string:token_employee>/<int:shift_id>', type="http", auth="user", website=True)
    ...
    @http.route('/odex25_planning/unassign/<string:token_employee>/<int:shift_id>', type="http", auth="user", website=True)
    def planning_self_unassign_with_user(self, token_employee, shift_id, **kwargs):
        ...
        if request.env.user:
            return redirect('/web?#action=planning.planning_action_open_shift')
        return request.env['ir.ui.view']._render_template('odex25_planning.slot_unassign')
