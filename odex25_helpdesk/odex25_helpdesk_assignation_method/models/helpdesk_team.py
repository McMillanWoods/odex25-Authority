...
class HelpdeskTicket(models.Model):
    ...
    def _onchange_ticket_type_values(self, team, ticket_type=None):
        return {
            'user_id': team.get_new_user(ticket_type).id,
        }
...
class HelpdeskTeam(models.Model):
    ...
    team_leader_id = fields.Many2one('res.users',domain=lambda self: [('groups_id', 'in', self.env.ref('odex25_helpdesk_security.group_helpdesk_normal_manager').id)])
    ...
    def get_new_user(self, ticket_type=None):
        ...
        if ticket_type:
            ...
            members_ids = sorted([member.member_id.id for member in self.members_ids])
        if members_ids:
            if self.assign_method == 'randomly':
                # randomly means new ticketss get uniformly distributed
                previous_assigned_user = self.env['odex25_helpdesk.ticket'].search([('team_id', '=', self.id)], order='create_date desc', limit=1).user_id
